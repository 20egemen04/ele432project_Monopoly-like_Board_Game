...
// 50MHz giriş ile 400kHz I2C clock elde etmek için: (50.000.000 / 400.000) / 2 = 62.5 -> 63
localparam [15:0] DIV_50MHZ = 16'd63;

//OLD.. localparam [15:0] DIV_100MHZ = 16'd125;         //desire 400KHz, have 100MHz, thus (1/(400*10^3)*100*10^6)/2, note div by 2 is for need to change in cycle
...
always@(posedge i_clk or negedge reset_n) begin
    if(!reset_n)
        ...
        clk_i2c_cntr <= 16'(clk_i2c_cntr + 1);
        if(clk_i2c_cntr == DIV_50MHZ-1) begin
            clk_i2c <= !clk_i2c;
            clk_i2c_cntr <= 0;
        end
    end
        ...
        case(state)
                ...
                if(byte_sent & cntr[0]) begin
                    ...
                    if(scl_is_low) begin
                        if(clk_i2c_cntr == DATA_HOLD_TIME) begin
                            {byte_sent, cntr} <= 4'({byte_sent, cntr} + 1);       //incr cntr, with overflow being caught (due to overflow, no need to set cntr to 0)
            ...
            SUB_ADDR: begin
                if(byte_sent & cntr[0]) begin
                    ...
                    if(scl_is_low) begin
                        if(clk_i2c_cntr == DATA_HOLD_TIME) begin
                            scl_is_low <= 1'b0;
                            {byte_sent, cntr} <= 4'({byte_sent, cntr} + 1);       //incr cntr, with overflow being caught
                            reg_sda_o <=  byte_sr[7];               //send MSB
                            byte_sr <= {byte_sr[6:0], 1'b0};        //shift out MSB
                        end
                    end
            ...
            READ: begin
                if(byte_sent) begin
                    ...
                    num_byte_sent <= 24'(num_byte_sent + 1);  //Incr number of bytes read
                    ack_in_prog <= 1'b1;
            ...
            WRITE: begin
                if(byte_sent & cntr[0]) begin
                    ...
                    if(scl_is_low) begin //negedge
                        if(clk_i2c_cntr == DATA_HOLD_TIME) begin
                            {byte_sent, cntr} <= 4'({byte_sent, cntr} + 1);
                            reg_sda_o <= byte_sr[7];
                            byte_sr <= {byte_sr[6:0], 1'b0};        //shift out MSB
                            scl_is_low <= 1'b0;
                        end
             ...
             */
            RELEASE_BUS: begin
                if(clk_i2c_cntr == DIV_50MHZ-3) begin
                    en_scl <= 1'b0;
                    state <= IDLE;
                    reg_sda_o <= 1'bz;
                    busy <= 1'b0;
